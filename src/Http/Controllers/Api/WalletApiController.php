...
{
    protected PassKitService $walletService;
    protected PassKitCrudManager $crudManager;

    public function __construct(
        PassKitService $walletService,
    ...
    public function getPrograms(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get loyalty programs', [
                'error' => $e->getMessage(),
    ...
    public function createProgram(CreateProgramRequest $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to create loyalty program', [
                'error' => $e->getMessage(),
                'data' => $request->validated()
            ]);

            return $this->errorResponse('Failed to create loyalty program', ['error' => $e->getMessage()], 500);
        }
    }

    /**
     * @api {get} /api/wallet/programs/{id} Get loyalty program
     * @apiName GetProgram
     * @apiGroup Programs
     * @apiVersion 1.0.0
     */
    public function getProgram(PassKitProgram $program): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get loyalty program', [
                'program_id' => $program->id,
                'error' => $e->getMessage()
            ]);

            return $this->errorResponse('Failed to retrieve loyalty program', [], 500);
        }
    }

    /**
     * @api {put} /api/wallet/programs/{id} Update loyalty program
    ...
    public function updateProgram(CreateProgramRequest $request, PassKitProgram $program): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to update loyalty program', [
                'program_id' => $program->id,
                'error' => $e->getMessage(),
                'data' => $request->validated()
            ]);

            return $this->errorResponse('Failed to update loyalty program', ['error' => $e->getMessage()], 500);
        }
    }

    /**
     * @api {delete} /api/wallet/programs/{id} Delete loyalty program
    ...
    public function deleteProgram(PassKitProgram $program): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to delete loyalty program', [
                'program_id' => $program->id,
                'error' => $e->getMessage()
            ]);

            return $this->errorResponse('Failed to delete loyalty program', ['error' => $e->getMessage()], 500);
        }
    }

    /**
     * @api {get} /api/wallet/members Get members
     * @apiName GetMembers
     * @apiGroup Members
     * @apiVersion 1.0.0
     */
    public function getMembers(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get members', [
                'error' => $e->getMessage(),
                'filters' => $request->all()
            ]);
    ...
    /**
     * @api {post} /api/wallet/members Create member
    ...
    public function createMember(CreateMemberRequest $request): JsonResponse
    {
        try {
            ...
            // Enroll in Shakewell Wallet service if sync is enabled
            if (config('passkit.auto_sync', true)) {
                try {
                    $walletResult = $this->walletService->enrollMember(
                        $member->tier_id,
                        [
                    ...
                    // Update with Wallet service ID
                    $member->update([
                        'passkit_id' => $walletResult['id'],
                        'last_sync_at' => now(),
                        'sync_pending' => false
                    ]);

                } catch (\Exception $syncError) {
                    Log::warning('Shakewell Wallet API: Member created locally but sync failed', [
                        'member_id' => $member->id,
            ...
            Log::error('Shakewell Wallet API: Failed to create member', [
                'error' => $e->getMessage(),
                'data' => $request->validated()
            ]);
    ...
    /**
     * @api {get} /api/wallet/members/{id} Get member
     * @apiName GetMember
     * @apiGroup Members
     * @apiVersion 1.0.0
     */
    public function getMember(PassKitMember $member): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get member', [
                'member_id' => $member->id,
                'error' => $e->getMessage()
            ]);
    ...
    /**
     * @api {put} /api/wallet/members/{id} Update member
    ...
    public function updateMember(UpdateMemberRequest $request, PassKitMember $member): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to update member', [
                'member_id' => $member->id,
                'error' => $e->getMessage(),
                'data' => $request->validated()
            ]);
    ...
    /**
     * @api {post} /api/wallet/members/{id}/points Update member points
    ...
    public function updateMemberPoints(Request $request, PassKitMember $member): JsonResponse
    {
        try {
            ...
            $result = $this->walletService->updateMemberPoints(
                $member->passkit_id,
                $request->points,
            ...
            Log::error('Shakewell Wallet API: Failed to update member points', [
                'member_id' => $member->id,
    ...
    /**
     * @api {post} /api/wallet/transactions Create transaction
    ...
    public function createTransaction(CreateTransactionRequest $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to create transaction', [
                'error' => $e->getMessage(),
                'data' => $request->validated()
            ]);
    ...
    /**
     * @api {get} /api/wallet/transactions Get transactions
    ...
    public function getTransactions(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get transactions', [
                'error' => $e->getMessage(),
                'filters' => $request->all()
            ]);
    ...
    /**
     * @api {get} /api/wallet/passes Get wallet passes
    ...
    public function getWalletPasses(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get wallet passes', [
                'error' => $e->getMessage(),
                'filters' => $request->all()
            ]);
    ...
    /**
     * @api {post} /api/wallet/passes Create wallet pass
    ...
    public function createWalletPass(Request $request): JsonResponse
    {
        try {
            $validator = Validator::make($request->all(), [
                'member_id' => 'required|string|exists:passkit_members,passkit_id',
            ...
            $member = PassKitMember::where('passkit_id', $request->member_id)->first();
            ...
            $walletPass = $this->walletService->createWalletPassWithUrls(
                $request->member_id,
                $request->user_id,
                $request->account_id,
            ...
            Log::error('Shakewell Wallet API: Failed to create wallet pass', [
                'error' => $e->getMessage(),
                'data' => $request->all()
            ]);
    ...
    /**
     * @api {get} /api/wallet/sync/status Get sync status
    ...
    public function getSyncStatus(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get sync status', [
                'error' => $e->getMessage(),
    ...
    /**
     * @api {post} /api/wallet/sync/trigger Trigger sync
     * @apiName TriggerSync
     * @apiGroup Sync
     * @apiVersion 1.0.0
     */
    public function triggerSync(Request $request): JsonResponse
    {
        try {
            ...
            if (in_array($entityType, ['members', 'all'])) {
                $memberResults = $this->walletService->syncAllMembers($accountId, [
                    'force' => $force,
                    'batch_size' => 50
                ]);
            ...
            if (in_array($entityType, ['wallet_passes', 'all'])) {
                $passResults = $this->walletService->syncAllWalletPasses($accountId, [
                    'force' => $force,
                    'batch_size' => 50
                ]);
            ...
            Log::error('Shakewell Wallet API: Failed to trigger sync', [
                'error' => $e->getMessage(),
                'account_id' => $request->account_id,
    ...
    /**
     * @api {get} /api/wallet/analytics Get analytics
    ...
    public function getAnalytics(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Shakewell Wallet API: Failed to get analytics', [
                'error' => $e->getMessage(),
                'account_id' => $request->account_id
            ]);
